                       ...
                       int num_blks, void *buf)
{
	...
	for (i = 0; i < num_blks; i++){
		...
		if (result != SUCCESS)
		{
			...
			{
				sd->disks[blk_disk]->ops->close(sd->disks[blk_disk]);
                sd->disks[blk_disk] = NULL;
				return result;
			}
			else if (result == E_BADADDR){
				return result;
                        ...
                        int num_blks, void *buf)
{
	...
	for (i = 0; i< num_blks; i++){
		...
		if (sd->disks[blk_disk] == NULL)
			return E_UNAVAIL;

		result = sd->disks[blk_disk]->ops->write(sd->disks[blk_disk], num, 1, current_buff_location);
...
/* helper function - reconstruct the data on the failed disk using
 * data on other disks and parity disk
 */
...
struct blkdev *raid4_create(int N, struct blkdev *disks[], int unit)
{
    ...
    r4dev->disk_size = d_size / unit * unit;
